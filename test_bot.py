...
def convert_price_to_text(amount):
    try:
        ...
        rubles_form = get_currency_form(rubles, ['рубль', 'рубля', 'рублей'])
        kopecks_form = get_currency_form(kopecks, ['копейка', 'копейки', 'копеек'])

        return f"{rubles} ({rubles_words}) {rubles_form} {kopecks_formatted} {kopecks_form}"
    ...
    try:
        ...
        if mode == "self_employed":
            await update.message.reply_text(
                ...
                "Услуга исполнителя (по ..., без 'не менее')\n"
                "Стоимость услуги\n"
                "Крайний срок оплаты\n"
                "Дата начала оказания услуг\n"
                "Дата конца оказания услуг\n"
                "Формат результата"
            )
...
async def collect_missing_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if context.user_data.get("waiting_for_input"):
        ...
        mode = context.user_data.get("mode")
        expected_count = 6 if mode == "self_employed" else 5
        ...
        if mode == "self_employed":
            context.user_data["additional_data"] = {
                ...
                "ДАТА НАЧАЛА": user_message[3],
                "ДАТА КОНЦА": user_message[4],
                "ФОРМАТ РЕЗУЛЬТАТА": user_message[5]
            }
            ...
            context.user_data["additional_data"] = {
                "УСЛУГА ИСПОЛНИТЕЛЯ": user_message[0],
                "СТОИМОСТЬ": user_message[1],
                "ДАТА НАЧАЛА": user_message[2],
                "ДАТА КОНЦА": user_message[3],
                "ФОРМАТ РЕЗУЛЬТАТА": user_message[4]
            }
...
async def process_and_send_files(update, context):
    ...
    try:
        cost_float = float(cost)
        cost_float2 = cost_float*5/(5+100)
        cost_text2 = convert_price_to_text(cost_float2)
        cost_with_text2 = f"{cost_text2}"
    except ValueError:
        cost_with_text2 = cost_float2
    
    replacement_dict = {
        "сокрФИО": card_data.get("ФИО(сокращенно)", ""),
        "ФИО": card_data.get("ФИО(полностью)", ""),
        "ИПполностью": card_data.get("ИП(полностью)", ""),
        "ИПсокращенно": card_data.get("ИП(сокращенно)", ""),
        ...
        "ИННБАНК": card_data.get("ИНН(банк)", ""),
        "КПП": card_data.get("КПП", ""),
        ...
        "ДАТА ВЫДАЧИ": card_data.get("Дата выдачи", ""),
        ...
        "ДАТА НАЧАЛА":additional_data.get("ДАТА НАЧАЛА",""),
        "ДАТА КОНЦА":additional_data.get("ДАТА КОНЦА","")
    }
